...
At the lowest level of detail, *treatment plans* define the parameters for treatment delivery.

Note that in radiotherapy, a *treatment plan* represents a concrete set of treatment delivery instructions prepared for a specific patient anatomy and machine (See also the [Radiotherapy Glossary](http://hl7.org/fhir/us/mcode/glossary.html). It is the most specific and most detailed concept in this model, whereas phases and courses course summarize what is requested or performed using typically more than one treatment plan. Radiotherapy treatment plans must not be confused with care plans that coordinate activities often at a much higher level. Here *treatment plan* is used as established in the radiotherapy domain, that is for a set of concrete treatment parameters that are requested to be applied as is in one or more sessions of radiotherapy treatment delivery. The concept for treatment plan in radiotherapy includes the lowest level of machine machine instructions to control a treatment machine. These details however, are not modelled in the FHIR representation of a radiotherapy treatment plan. Instead, the respective FHIR profiles include references to DICOM artifacts (DICOM treatment plans and DICOM treatment records) that contain treatment parameters down to the machine geometry and motion during treatment.
...
The following figure shows the resource profiles to represent the radiotherapy requests and procedures at different levels. Note that the radiotherapry prescriptions and plans represent instances of patient specific requests, not templates or general definitions of protocols. They are therefore modelled as ServiceRequest. These requests may be embedded in an even higher level CarePlan that coordinates various activities of radiotherpy, but radiotherapy CarePlans are not scope of this IG.

- *Radiotherapy Prescriptions* (left column) are *original orders* (ServiceRequest.intent = original-order). Prescriptions can cover an original request for a single Treatment Plan (Radiotherapy Plan Prescription), or cumulative request for a Phase (Radiotherapy Phase Prescription) or for a complete Course (adiotherapy Course Prescription).
- *Radiotherapy Plans* (middle column) are *filler orders* (ServiceRequest.intent = original-order) that represent how the radiotherapy system will fulfill the prescribed radiotherapy treatment. Treatment plans can be summarized on different levels to cover a single plan (Radiotherapy Treatment Plan), the sum of plans for a complete phase (Radiotherapy Planned Phase), or the sum of plans for the entire course of treatment (Radiotherapy Planned Course).
- *Raditherapy Procedures* (right column) document the treatment that was actually performed on different levels of detail (Radiotherapy Course Summary, Radiotherapy Treated Phase, Radiotherapy Treated Plan).
